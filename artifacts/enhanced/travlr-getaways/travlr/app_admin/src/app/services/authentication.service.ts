...
import { Observable } from 'rxjs';  // Ensure you're importing Observable
...
export class AuthenticationService {
  ...
  //  NEW: Get the user's role from the JWT
  ...
  // information.
  // Login method, returns Observable<AuthResponse> for component to subscribe to
  public login(user: User, passwd: string): Observable<AuthResponse> {
  return new Observable(observer => {
    this.tripDataService.login(user, passwd).subscribe({
      next: (value: AuthResponse) => {
        if (value && value.token) {
          this.saveToken(value.token);   // ⭐ THIS is the missing piece
        }
        observer.next(value);
        observer.complete();
      },
